...
static BOOL svmDirectoryHasEntries(NSString *path) {
    NSArray *items = [NSFileManager.defaultManager contentsOfDirectoryAtPath:path error:nil];
    return items.count > 0;
}

static BOOL svmFileContainsCaseInsensitiveToken(NSString *path, NSArray<NSString *> *tokens) {
...
static NSString *svmCurrentProfileGameDir() {
...
static BOOL svmGameDirLooksShaderHeavy(NSString *gameDir) {
    if (gameDir.length == 0) return NO;

    if (svmDirectoryHasEntries([gameDir stringByAppendingPathComponent:@"shaderpacks"])) {
        return YES;
    }

    NSArray<NSString *> *mods = [NSFileManager.defaultManager contentsOfDirectoryAtPath:[gameDir stringByAppendingPathComponent:@"mods"] error:nil];
    NSArray<NSString *> *shaderModHints = @[
        @"iris", @"oculus", @"optifine", @"embeddium", @"rubidium", @"sodium", @"shader"
    ];
    ...
    if (svmFileContainsCaseInsensitiveToken([gameDir stringByAppendingPathComponent:@"optionsof.txt"], @[@"shaderpack", @"ofshaders"])) {
        return YES;
    }
    if (svmFileContainsCaseInsensitiveToken([gameDir stringByAppendingPathComponent:@"optionsshaders.txt"], @[@"shaderpack=", @"packname"])) {
        return YES;
    }
    return NO;
}
...
- (instancetype)initWithMetadata:(NSDictionary *)metadata {
    ...
    if (![renderer isKindOfClass:NSString.class] || renderer.length == 0) {
        renderer = getPrefObject(@"video.renderer");
    }
    BOOL shaderWorkload = svmGameDirLooksShaderHeavy(svmCurrentProfileGameDir());
...
- (void)viewDidLoad
{
    ...
    if (@available(iOS 15.0, tvOS 15.0, *)) {
        ...
        } else if(getPrefBool(@"video.max_framerate")) {
