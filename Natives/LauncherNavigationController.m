...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        MinecraftResourceDownloadTask *finishedTask = self.task;
        self.progressViewMain.observedProgress = nil;
        if (finishedTask.metadata) {
            __block NSDictionary *metadata = finishedTask.metadata;
            [self invokeAfterJITEnabled:^{
            ...
            NSString *installerPath = finishedTask.pendingModLoaderInstallerPath;
            if (installerPath.length > 0 && [NSFileManager.defaultManager fileExistsAtPath:installerPath]) {
                [self invokeAfterJITEnabled:^{
                    [self enterModInstallerWithPath:installerPath hitEnterAfterWindowShown:YES];
                }];
            }
        }
        self.task = nil;
