...
static BOOL directoryHasEntries(NSString *path) {
    NSArray *items = [NSFileManager.defaultManager contentsOfDirectoryAtPath:path error:nil];
    return items.count > 0;
}

static BOOL fileContainsCaseInsensitiveToken(NSString *path, NSArray<NSString *> *tokens) {
...
static NSString *resolveGameDirectoryForLaunchTarget(id launchTarget) {
...
static BOOL gameDirLooksShaderHeavy(NSString *gameDir) {
    if (gameDir.length == 0) return NO;

    NSString *shaderpackDir = [gameDir stringByAppendingPathComponent:@"shaderpacks"];
    if (directoryHasEntries(shaderpackDir)) {
        return YES;
    }

    NSString *modsDir = [gameDir stringByAppendingPathComponent:@"mods"];
    NSArray<NSString *> *mods = [NSFileManager.defaultManager contentsOfDirectoryAtPath:modsDir error:nil];
    ...
    NSString *optionsOfPath = [gameDir stringByAppendingPathComponent:@"optionsof.txt"];
    if (fileContainsCaseInsensitiveToken(optionsOfPath, @[@"shaderpack", @"ofshaders"])) {
        return YES;
    }
    NSString *optionsShadersPath = [gameDir stringByAppendingPathComponent:@"optionsshaders.txt"];
    if (fileContainsCaseInsensitiveToken(optionsShadersPath, @[@"shaderpack=", @"packname"])) {
        return YES;
    }
    return NO;
}
...
void init_loadDefaultEnv() {
    ...
    // Keep Mesa shader disk cache on persistent storage to reduce shader stutter/freezes.
    const char *gameDir = getenv("POJAV_GAME_DIR");
    if (gameDir) {
        NSString *cacheDir = [NSString stringWithFormat:@"%s/cache/mesa_shader_cache", gameDir];
...
int launchJVM(NSString *username, id launchTarget, int width, int height, int minVersion) {
    ...
    if ([launchTarget isKindOfClass:NSDictionary.class]) {
        ...
        BOOL shaderWorkload = gameDirLooksShaderHeavy(resolveGameDirectoryForLaunchTarget(launchTarget));
        setenv("POJAV_SHADER_WORKLOAD", shaderWorkload ? "1" : "0", 1);
        if (shaderWorkload) {
        ...
        launchJar = YES;
        setenv("POJAV_SHADER_WORKLOAD", "0", 1);
        unsetenv("GALLIUM_DRIVER");
        unsetenv("MESA_LOADER_DRIVER_OVERRIDE");
    }
